...
const uploadCustomerData = async (req, res) => {
    ...
    try {
        ...
        worksheetData.eachRow(async (row, rowNumber) => {
            if (rowNumber > 1) {
                const [, customer_id, first_name, last_name, age, phone_number, monthly_salary, approved_limit] = row.values;
        ...
        res.status(201).json({ message: "Excel Sheet data are transferred to the MySQL database successfully" });
    } catch (error) {
